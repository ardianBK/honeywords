...
class SHA3_256
{
    ...
    {
       return self::keccak_32($password, 256, 256, 0x06);
    }

	private static function keccak_32($password, $capacity, $outputlength, $suffix)
    {
       // echo 'capacity awal = '.$capacity.'</br>';
        $capacity /= 8;
       // echo 'capacity = '.$capacity.'</br>';

        //$inlen = self::ourStrlen($password);
        $inlen = mb_strlen($password, '8bit'); //dapat jumlah string utk -> 8 bit ->>>>>> INPUT LENGHT byte 3 -> abc
      //  echo 'inlen = '.$inlen.'</br>';

        $rsiz = 200 - 2 * $capacity; //mencari panjang padding -> abc -> 136 byte
      //  echo 'rsiz = '.$rsiz.'</br>';
        $rsizw = $rsiz / 8;
     //   echo 'rsizw = '.$rsizw.'</br>';

        $st = [];
        for ($i = 0; $i < 25; $i++) { //buat array dalam array
            $st[] = [0, 0, 0, 0];
       //     echo '<pre>';print_r($st);
      //      echo '</pre>';
        }

        //inisialisasi block awal
        for ($in_t = 0; $inlen >= $rsiz; $inlen -= $rsiz, $in_t += $rsiz) {
            for ($i = 0; $i < $rsizw; $i++) {

                $subrt = mb_substr($password, $i * 8 + $in_t, 8);
                $t = unpack('v*', $subrt);
        ...
        $temp = mb_substr($password, $in_t, $inlen);
        ...
        for ($i = 0; $i < $rsizw; $i++) {
            $t = unpack('v*', mb_substr($temp, $i * 8, 8));
       ...
       $r = mb_substr($out, 0, $outputlength / 8);
      // echo '</br> ini r = '.$r;
       $kata = bin2hex($r);
       $hasil = substr($kata, 0, 64);
       return $hasil;
    }
